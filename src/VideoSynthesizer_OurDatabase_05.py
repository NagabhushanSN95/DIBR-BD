...
# Copied from VSR005/VideoSynthesizer_OurDatabase_05.py, and updated for this model.
...
class DibrModel:
    ...
    def load_data(self, video_name, frame1_num):
        input_data = self.data_loader.load_test_data(video_name, frame1_num, patch_start_pt=None)
        return input_data
...
def start_testing(configs: dict):
    ...
    database_dirpath = Path('../Data/Databases/Ours/Data/')
    ...
    for row_num in range(num_frames):
        ...
        video_name, frame3_num = frame_data
        frame3_num = int(frame3_num)

        video_dirpath = database_dirpath / f'{group}/{video_name}'
        frame3_path = video_dirpath / f'render/rgb/{frame3_num:04}.png'
        frame5_path = video_dirpath / f'render/rgb/{frame3_num + 2:04}.png'

        video_output_dirpath = output_dirpath / video_name
        ...
        infilled_frame4_path = pred_frames_dirpath / f'{frame3_num + 1:04}.png'
        output_frame5_path = pred_frames_dirpath / f'{frame3_num + 2:04}.png'
        ...
        if not infilled_frame4_path.exists():
            input_data = dibr_model.load_data(video_name, frame3_num)
